...
Production evaluation does not require manually entered points. For each condition and evaluation budget it emits one aggregate curve point: x is that budget's configured generated-token cap per example, independently of its tool/iteration cap; y is aggregate strict or lenient benchmark score. Actual mean and total generation remain separate efficiency measurements. Production caps are 1k/3k/6k/10k and the root-agent per-turn maximum is 1k. Startup validates each cap against the real maximum of `(tool calls + 1 final generation) × per-turn maximum`, so impossible labels fail before a model runner starts. The historical 3k anchor is therefore reachable. The standalone `expertise --point` command remains only as a debugging utility.
...
The current schema-v4 provenance contract cryptographically binds artifact content, effective protocol and command configuration identities, canonical corpus/evaluation snapshots, portable parent dependencies, and the actual local model/tokenizer snapshot contents whenever those resources are loaded. The historical corpus audit produced 371 decoded text documents, 2,571 non-overlapping semantic units, an estimated 513,596 eligible lexical tokens, 4,567 symbol occurrences, 14,048 syntactic call sites, and 1,733 conservatively verified call relations. It reports 90 ambiguous, 2,953 lexically shadowed, and 9,272 otherwise excluded call sites. These are tokenizer-independent historical audit estimates; actual model-token exposure must be reported during training.

CPU/mock validation currently collects 67 tests. It additionally covers effective CLI protocol hashing, anchor-spanning and runtime-reachable expertise budgets, exact study-bank model/tokenizer snapshot enforcement, synthetic-bank rejection (including official memory dependencies), actual model/tokenizer snapshot mutation, and relocation of a complete corpus/manifest/study bundle while retaining closed-snapshot checks.

All older checked-in record, replay, latent, PEEK, root-agent, and real-Qwen result artifacts predate the current artifact contract and are historical only. Production loaders reject them rather than silently treating them as current. In particular, PEEK-64 and the real-Qwen latent optimization smoke remain blocked; Round 3.1 does not weaken either gate. The current infrastructure status is in [REPAIR_ROUND_3_1_REPORT.md](docs/REPAIR_ROUND_3_1_REPORT.md).
